            ...
            <div class="col-md-8 col-md-offset-2">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <form enctype="multipart/form-data" method="POST" id="form" action="{{ route('deportes.update', ['deporte'=>$deporte->id]) }}">
                            {{ method_field('PATCH') }}
                            @csrf
                            <div class="form-group">
                                <label for="modality">Nombre Deporte</label>
                                <input type="text" class="form-control" id="modality" name="modality"
                                    placeholder="Nombre del deporte" value="{{ $deporte->modality }}">
                            </div>
                            @if (\Storage::disk('deportes_images')->has($deporte->path))
                                <img class="card-img-top" class="w-100" src="{{ url('/imagen_deportes/' . $deporte->path) }}" alt="Card image cap">
                            @endif
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label for="instructor" style="font-weight: bold">Persona a cargo: </label>
                                <p style="display: inline-block; margin-left: 5px">{{ $deporte->instructor->first_name }} {{ $deporte->instructor->last_name }}</p>
                            ...
                            <div>
                                <h6 class="my-3 font-weight-bold">Editar horarios</h6>
                                <div class="px-3" id="content">
                                    @foreach ($deporte->schedules as $schedule)
                                        <p class="d-inline-block" id={{ $schedule->id }}> {{ $schedule->day }} {{ $schedule->strat }} - {{ $schedule->end }} </p>
                            ...
                            <div class="form-group">
                                <label for="#">Agregar Horarios</label>
                            ...
                            @php
                                $total_ = count($deporte->schedules);
                            @endphp
                            <input type="hidden" name="totalhorarios" id="totalhorarios" value="{{ $total_ }}">
                            <div class="form-group" id="hiddenschudeles">
                                ...
                                @foreach ($deporte->schedules as $schedule)
                                    @php
                                        $index_ ++;
                                    @endphp
                                    <input type="hidden" name="schedule{{ $index_ }}" value="{{ $schedule->id }}">
                                @endforeach
                            </div>
                        </form>
...
@section('scripts')
    <script>
        $(document).ready(function() {
            ...
            $('#hiddenschudeles').children().each(function(){
                lista_schedules.push(parseInt($(this).val()))
            })
            var i = lista_schedules.length
            ...
            $('#save').on('click', (e) => {
                ...
                if (lista_schedules.includes(id_selected)){
                    ...
                    p.setAttribute("class","d-inline-block")
                    content = document.getElementById("content")
                    content.appendChild(p)
                    a = document.createElement('a')
                    img = document.createElement('img')
                    img.setAttribute("src","{{ asset('images/icons/delete.png') }}")
                    a.setAttribute("class","ml-2")
                    a.setAttribute("href","#")
                    a.appendChild(img)
